...
function Get-CodexMaintenanceState
{
    ...
    $blockReason = if ($LinkStatus -eq "Unsafe") {
        "UnsafeLink"
    } elseif (!$ProcessQuerySucceeded) {
        "ProcessQueryFailed"
    } elseif ($injectionRequired -and $targetMirrorRunning) {
    ...
    $blockDetail = if ($blockReason -eq "UnsafeLink") {
        "The live tweak path is unsafe and will not be replaced."
    } elseif ($blockReason -eq "ProcessQueryFailed") {
